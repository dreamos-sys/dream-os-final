/**
 * Dream OS v1.3.1 - Logo Tap Fix + Counter
 */
window.DreamOS = {
    ...
    run(mod, fn, ...args) { try { if (this.modules[mod]?.[fn]) return this.modules[mod][fn](...args); } catch (e) { console.error(e); } },    t(key, lang) { const l = lang || localStorage.getItem('dream-lang') || 'id'; return this.translations[l]?.[key] || this.translations['id']?.[key] || key; }
};
...
DreamOS.register('home', {
    ...
    renderMenuGrid() {        const c = document.getElementById('menu-grid'); if (!c) return;
    ...
    initNav() {
        document.querySelectorAll('#bottom-nav .nav-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                if (t === 'config') DreamOS.run('i18n', 'showPicker');
                else if (t === 'menu') alert('📋 Quick Menu: Search, QR, Settings');
            });
        });
    }
});
...
DreamOS.register('i18n', {
    setLanguage(lang) {
        ...
        document.querySelectorAll('[data-i18n]').forEach(el => { if (DreamOS.translations[lang][el.getAttribute('data-i18n')]) el.textContent = DreamOS.translations[lang][el.getAttribute('data-i18n')]; });
...
DreamOS.register('ghost', {
    init() {
        ...
        if (logo) {
            // Gunakan 'touchstart' untuk respons lebih cepat di HP, 'pointerdown' sebagai fallback
            const handleTap = (e) => {
                e.preventDefault(); // Mencegah zoom/scroll saat tap cepat
                ...
                if (DreamOS.state.tapCount === 1) {
                    DreamOS.state.tapTimer = setTimeout(() => { DreamOS.state.tapCount = 0; if(counter) counter.style.opacity = '0'; }, 2000);
                }
                
                if (DreamOS.state.tapCount >= 7) {
                    clearTimeout(DreamOS.state.tapTimer);
                    DreamOS.state.tapCount = 0;
                    if(counter) { counter.textContent = 'GHOST MODE ACTIVATED!'; counter.style.color = '#ef4444'; }
                    setTimeout(() => {
    ...
    toggleEruda() {
        if (DreamOS.state.erudaActive) {
            ...
            DreamOS.state.erudaActive = false; alert('✅ Eruda removed');
        } else {
            const s = document.createElement('script');
            s.src = "https://cdn.jsdelivr.net/npm/eruda";
            s.onload = () => { if (typeof eruda !== 'undefined') { eruda.init(); DreamOS.state.erudaActive = true; alert('✅ Eruda injected!'); } };
