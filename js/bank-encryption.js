 ...
 * IMPROVED: Dynamic salt dari config
 */
const BankEncryption = {
  ...
  async getKey() {
    ...
    const salt = (window.__DREAMOS_CONFIG && window.__DREAMOS_CONFIG.bankSalt)
      ? window.__DREAMOS_CONFIG.bankSalt
      : 'bank-salt-fallback-' + (u.id || 'system');
    
    const k = await crypto.subtle.importKey(
      'raw', m.encode((u.id || 'system') + '-dreamos-bank-v3'),
      { name: 'PBKDF2' }, false, ['deriveKey']
    );
    this._key = await crypto.subtle.deriveKey(
      { name: 'PBKDF2', salt: m.encode(salt), iterations: 200000, hash: 'SHA-256' },
...
console.log('🏦 AES-256-GCM Encryption Ready (dynamic salt)');
