 ...
 * Enkripsi semua data sensitif di localStorage
 */
const BankEncryption = {
  ...
  getEncryptionKey() {
    ...
    return (user.id || 'system') + '-dreamos-secret';
  },

  async secureSet(key, value) {
    const json = JSON.stringify(value);
    const encrypted = await this.encrypt(json, this.getEncryptionKey());
  ...
  async secureGet(key) {
    ...
    try {
      const json = await this.decrypt(encrypted, this.getEncryptionKey());
